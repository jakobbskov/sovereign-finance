...
class FinanceCoreAuthModeGuardTest(unittest.TestCase):
    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory()
        self.data_dir = Path(self.tmp.name)
        ...
        self._write_json(self.app_module.FINANCE_PATH, {"version": 1, "income": [], "fixed_expenses": [], "events": [], "strategies": []})
        self._write_json(self.app_module.EVENTS_PATH, {"version": 1, "events": []})
        self._write_json(self.app_module.DECISIONS_PATH, {"version": 1, "decisions": []})
    ...
    def test_core_mode_accepts_valid_core_auth_session(self):
        with patch.object(
            self.app_module,
            "get_current_core_auth_user",
            return_value=("ok", {"user_id": "core-user", "username": "jakob", "role": "admin"}),
    ...
    def test_core_mode_rejects_missing_or_invalid_auth(self):
        with patch.object(
            self.app_module,
            "get_current_core_auth_user",
            return_value=("unauthorized", None),
    ...
    def test_core_mode_ignores_existing_local_finance_session(self):
        with self.client.session_transaction() as session:
            session["finance_auth"] = True
    ...
    def test_core_mode_disables_local_login_post(self):
        response = self.client.post("/login", data={"password": "test-password"})

        self.assertEqual(response.status_code, 401)
        self.assertIn("deaktiveret", response.get_data(as_text=True))

    def test_core_mode_whoami_reports_core_session(self):
        with patch.object(
            self.app_module,
            "get_current_core_auth_user",
            return_value=("ok", {"user_id": "core-user", "username": "jakob", "role": "admin"}),
